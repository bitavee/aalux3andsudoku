...
constexpr int kPreviewHeightPercent = 30;
...
void FontSelectionActivity::onEnter() {
  ...
  previewHeight = usableHeight * kPreviewHeightPercent / 100;
...
void FontSelectionActivity::loop() {
  ...
  const int rowHeight = metrics_.listRowHeight > 0 ? metrics_.listRowHeight : 1;
...
void FontSelectionActivity::renderPreviewPane(int top, int height, int fontId, const char* fontName) const {
  const int left = kPreviewPadding;
  const int width = renderer.getScreenWidth() - (kPreviewPadding * 2);
  if (width <= 0 || height <= 0) return;

  const int labelFontId = UI_10_FONT_ID;
  const int labelH = renderer.getTextHeight(labelFontId);
  const int labelGap = 4;
  const int labelReserved = labelH + labelGap + kPreviewPadding;

  char labelBuf[128];
  snprintf(labelBuf, sizeof(labelBuf), "%s \"%s\"", tr(STR_PREVIEW), fontName ? fontName : "");
  const int labelY = top + height - kPreviewPadding - labelH;
  ...
  const int innerHeight = height - kPreviewPadding - labelReserved;
