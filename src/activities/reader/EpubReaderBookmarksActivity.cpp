...
int EpubReaderBookmarksActivity::getListItemsPerPage(const GfxRenderer& renderer) {
  const int rowHeight = UITheme::getInstance().getMetrics().listWithSubtitleRowHeight;
  return rowHeight > 0 ? getListHeight(renderer) / rowHeight : 1;
}
...
void EpubReaderBookmarksActivity::loop() {
  ...
  buttonNavigator.onNextContinuous([this] {
    selectorIndex = ButtonNavigator::nextPageIndex(selectorIndex, bookmarks.size(), getListItemsPerPage(renderer));
    requestUpdate();
  });

  buttonNavigator.onPreviousContinuous([this] {
    selectorIndex = ButtonNavigator::previousPageIndex(selectorIndex, bookmarks.size(), getListItemsPerPage(renderer));
    requestUpdate();
  });
}
