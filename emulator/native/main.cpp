...
struct Framebuffer {
  ...
  void fillRect(int x, int y, int w, int h, bool white) {
    for (int dy = 0; dy < h; ++dy)
      for (int dx = 0; dx < w; ++dx) setPixel(x + dx, y + dy, white);
  }
...
static std::string humanSize(uint64_t n) {
  ...
  if (n < 1024)
    ...
    std::snprintf(buf, sizeof(buf), "%.1f GB", n / (1024.0 * 1024 * 1024));
  return buf;
}
...
struct Server {
  ...
  void sendInitialState(int fd) {
    ...
    std::string cfgMsg =
        "{\"type\":\"config\",\"width\":" + std::to_string(lw) + ",\"height\":" + std::to_string(lh) + "}";
  ...
  void handleClientMessage(const std::string& payload) {
    ...
    if (type == "button") {
      ...
      if (btn == "SIDE_UP")
        ...
        ui.moveSelection(-ui.visibleRows);
      else if (btn == "FRONT_4")
        ui.moveSelection(ui.visibleRows);
      dirty = true;
    } else if (type == "rescan") {
  ...
  void serveClient(int fd) {
    ...
    if (req->method == "GET" && req->target == "/ws" && isWs) {
      ...
      if (key.empty()) {
        sendHttpResponse(fd, 400, "Bad Request", "text/plain", "missing key");
        ::close(fd);
        return;
      }
      ...
      if (!sendAll(fd, os.str().data(), os.str().size())) {
        ::close(fd);
        return;
      }
      ...
      while (!shuttingDown) {
        ...
        if (opcode == 0x9) {
          wsSendPong(fd, payload.data(), payload.size());
          continue;
        }  // ping
    ...
    if (req->method == "GET") {
      ...
      if (path.empty()) {
        sendHttpResponse(fd, 400, "Bad Request", "text/plain", "bad path");
        ::close(fd);
        return;
      }
