...
- **Grayscale covers.** Book covers render in 4-level grayscale (black / dark / light / white), composited over the e-ink base frame via LSB/MSB plane passes, on the Carrousel, home hero, and Bookshelf. Thumbnails are generated at their exact on-screen size and Atkinson-dithered for tonal depth instead of flat 1-bit. Existing covers regenerate on first view. Trade-off: grayscale uses a full-refresh composite, so navigating covers flashes more than the old 1-bit covers did.
